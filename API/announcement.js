...
export default async function handler(req,res){
  try {
    if(req.method === "GET"){
      const response = await fetch(`https://api.github.com/repos/$cidler6901/$proxy/contents/$annoucement.html`, {
    ...
    if(req.method === "POST"){
      ...
      const response = await fetch(`https://api.github.com/repos/$cidler6901/$proxy/contents/$annoucement.html`, {
        ...
        body: JSON.stringify({ message: enabled?"Update announcement":"Remove announcement", content, sha, branch: BRANCH })
      });
